...
void gen_monsters(dungeon *d)
{
  ...
  for (i = 0; i < d->num_monsters; i++) {
    ...
    while ((d->monster_descriptions[rand_desc].get_abilities() & NPC_UNIQ) && 
          (d->monster_descriptions[rand_desc].generated > 0) &&
          d->monster_descriptions[rand_desc].get_rarity() < rand_rarity) {
      rand_desc = rand() % size;
